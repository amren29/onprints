...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    setSocial({ facebook: ss.facebook, instagram: ss.instagram, tiktok: ss.tiktok, twitter: ss.twitter })
  }, [])

  const hasSocial = social.facebook || social.instagram || social.tiktok || social.twitter
  ...
  return (
    <footer className="bg-gray-950 text-gray-400">
      <div className="max-w-6xl mx-auto px-4 py-14 grid grid-cols-2 md:grid-cols-4 gap-10">
        ...
        <div className="col-span-2 md:col-span-1">
          ...
          <p className="text-sm leading-relaxed mb-4">
            {g.tagline || 'Professional printing services with online artwork proofing.'}
          </p>
          {g.contactPhone && <p className="text-xs text-gray-500 mb-1">{g.contactPhone}</p>}
          {g.contactAddress && <p className="text-xs text-gray-600 mb-1">{g.contactAddress}</p>}
          <a href={`mailto:${g.contactEmail}`} className="text-xs text-accent hover:underline mt-1 block">
            {g.contactEmail}
          </a>
          {hasSocial && (
            <div className="flex items-center gap-3 mt-4">
              {social.facebook && (
                <a href={social.facebook} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition" aria-label="Facebook">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
              )}
              {social.instagram && (
                <a href={social.instagram} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition" aria-label="Instagram">
              ...
              {social.tiktok && (
                <a href={social.tiktok} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition" aria-label="TikTok">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 0010.86 4.46V13.2a8.16 8.16 0 005.58 2.2V12a4.85 4.85 0 01-3.77-1.54V6.69h3.77z"/></svg>
                </a>
              )}
              {social.twitter && (
                <a href={social.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition" aria-label="Twitter">
      ...
      <div className="border-t border-gray-900 text-center text-xs py-5 text-gray-600 px-4">
        <span>&copy; {new Date().getFullYear()} {g.shopName} Sdn Bhd. All rights reserved.</span>
        <span className="mx-3">·</span>
        <span>Printed with care in Malaysia</span>
      </div>
    </footer>
  )
}
