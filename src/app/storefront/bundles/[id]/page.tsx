...
import { getBundleById, updateBundle } from '@/lib/db/bundles'
...
export default function EditBundlePage() {
  ...
  const { data: bundle, isLoading } = useQuery({
    queryKey: ['bundles', shopId, id],
    queryFn: () => getBundleById(shopId, id),
    enabled: !!shopId && !!id,
  })
  ...
  if (bundle && !hydrated) {
    ...
    setItems((bundle.items as any[]).map((i: any) => ({ catalogId: i.catalogId ?? '', name: i.name, qty: i.qty })))
    setDiscountType(bundle.discount_type)
    setDiscountValue(bundle.discount_value)
    setOriginalPrice(bundle.original_price)
  ...
  const updateMut = useMutation({
    mutationFn: () => updateBundle(shopId, id, {
  ...
  if (!isLoading && !bundle) {
    return (
      <MyStoreShell>
        ...
        <div className="empty-state" style={{ paddingTop: 80 }}>Bundle not found</div>
      </MyStoreShell>
    )
  }
