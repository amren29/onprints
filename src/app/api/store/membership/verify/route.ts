...
export async function GET(req: NextRequest) {
  try {
    ...
    // Extract tier info from description or reference
    const ref = bill.reference_1 || ''
    const tierIdMatch = ref.match(/^membership-(.+?)-\d+$/)

    return NextResponse.json({
      verified: true,
      tierId: tierIdMatch?.[1] || null,
      tierName: bill.description?.replace(' — 12 months', '') || null,
